...
def plot_geodata(counties_data, plot_title, legend_handles=None):  # New plotting function
    ...
    #ax.text(0.80, 0.90, 'Minnesota', fontsize=24, ha='right', va='top', transform=ax.transAxes)
...
for index, row in bees.iterrows():
    ...
    populate_occurrences(row, counties_data)  # modifies .color directly
    plot_geodata(counties_data, row['Scientific Name']) 
